...
-record(state, {
    operation_id :: bank_api:operation_id(),
    logic_handler :: atom(),
...
-spec init(Req :: cowboy_req:req(), Opts :: bank_router:init_opts()) ->
...
allowed_methods(
    Req,
    State = #state{
        operation_id = 'CreateAccountHolder'
    }
) ->
    {[<<"POST">>], Req, State};
...
handle_request_json(
    ...
    case bank_api:populate_request(OperationID, Req0, ValidatorState) of
        {ok, Populated, Req1} ->
            {Code, Headers, Body} = bank_logic_handler:handle_request(
                LogicHandler,
                OperationID,
                Req1,
                maps:merge(State#state.context, Populated)
            ),
            _ = bank_api:validate_response(
                OperationID,
                Code,
                Body,
                ValidatorState
